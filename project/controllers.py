...
import validate
import verify
from models import database, Fleet, EventSignature, EventInput,\
    EventOutput, Event, StarLog, StarLogEventSignature,\
    ChainIndex, Chain
import json
import traceback


class StarLogController:
    def get_starlogs(self, previous_hash, before_time,
    ...
    def add_starlog(self, requestData):
        ...
        try:
            ...
            star_log_json = json.loads(requestData)
            ...
            if not is_genesis:
                previous_chain = session.query(ChainIndex) \
                    .filter_by(hash=previous_hash).first()
                if previous_chain is None:
                    raise ValueError('previous starlog with hash '
                                     '%s cannot be found' % previous_hash)

            chain_index = session.query(ChainIndex) \
                .filter_by(hash=star_log_json['hash']).first()
            if chain_index:
                raise ValueError('starlog with hash %s already exists'
                                 % chain_index.hash)

            highest_chain = session.query(Chain) \
                .order_by(Chain.chain.desc()).first()
            root_id = None
            ...
            chain_index = ChainIndex(root_id, previous_chain_id, None, previous_star_log_id, star_log_json['hash'],
                                     star_log_json['previous_hash'], height, chain_count)
            ...
            previous_star_log = session.query(StarLog).filter_by(
                id=previous_star_log_id).first() if previous_star_log_id == 0 or previous_star_log_id is not None else None
            interval_id = None
            ...
            for current_event in star_log_json['events']:
                ...
                if new_signature:
                    event_signature = EventSignature(util.get_event_type_id(current_event['type']), fleet.id,
                                                     current_event['hash'], current_event['signature'], util.get_time(),
                                                     1)
                ...
                for current_input in current_event['inputs']:
                    ...
                    for input_uses_signature in input_uses_signatures:
                        for signatureBind in session.query(StarLogEventSignature).filter_by(
                                event_signature_id=input_uses_signature).all():
                    ...
                    for signature_star_log in signature_star_logs:
                        signature_chain_index = session.query(ChainIndex).filter_by(
                            star_log_id=signature_star_log).first()
                        ...
                        while next_chain_index is not None:
                            ...
                            if next_chain_index.chain == chain_index.chain:
                                raise Exception('event %s has already been used by %s' % (
                                current_input['key'], next_chain_index.hash))
                            next_chain_index = None if next_chain_index.root_id is None else session.query(
                                ChainIndex).filter_by(id=next_chain_index.root_id).first()
                    ...
                    origin_binds = session.query(StarLogEventSignature).filter_by(
                        event_signature_id=input_origin.event_signature_id).all()
                    validated_origin = False
                    origin_checked_chains = []
                    for origin_bind in origin_binds:
                        origin_chain_index = session.query(ChainIndex).filter_by(
                            star_log_id=origin_bind.star_log_id).first()
                ...
                for current_output in current_event['outputs']:
                    ...
                    if new_signature:
                        ...
                        if current_output['star_system']:
                            target_star_system = session.query(StarLog).filter_by(
                                hash=current_output['star_system']).first()
                        ...
                        target_output = Event(current_output['key'], util.get_event_type_id(current_output['type']),
                                              output_fleet.id, current_output['count'], target_star_system_id)
                        ...
                        event_output = session.query(EventOutput).filter_by(index=current_output['index'],
                                                                            event_id=target_output.id,
                                                                            event_signature_id=event_signature.id)
                    # Append this for further validation.
...
class EventController:
    ...
    def add_events(self, requestData):
        session = database.session()
        try:
            validate.byte_size(util.maximumEventSize(), requestData)
            event_json = json.loads(requestData)
            ...
            event_signature = EventSignature(util.get_event_type_id(event_json['type']), fleet.id, event_json['hash'],
                                             event_json['signature'], util.get_time(), 0)
            session.add(event_signature)
            session.flush()
            inputs = []
...
class ChainController:
    def get_chains(self, limit, height):
        session = database.session()
        ...
        for match in matches:
            ...
            for signature_bind in signature_binds:
                ...
                for current_output in output_events:
                    ...
                    # Rewards sent to the probed system can't have been known, so they would be left blank.
                    output_star_system_hash = None if output_star_system.hash == match.hash else output_star_system.hash
                    outputs.append(current_output.get_json(util.get_event_type_name(current_output_event.type_id),
                                                           output_fleet.hash, current_output_event.key,
                                                           output_star_system_hash, current_output_event.count))
                events.append(
                    signature_match.get_json(fleet.hash, fleet.public_key, inputs, outputs, signature_bind.index))
            results.append(match.get_json(events))
        return results